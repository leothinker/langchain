...
class MoviesApi:
    ...
    HttpHeader = {"Content-Type": "application/json", "Authorization": "Bearer YOUR_ACCESS_TOKEN"}
    ...
    @tool
    def get_movies_movies_get(year: int | None, sort: bool | None) -> list:
    ...
    def create_movie_movies_post(title: str, year: int) -> dict:
        ...
        response = requests.post(
            MoviesApi.BaseUrl + "/movies", headers=MoviesApi.HttpHeader, json=data
        )
        if response.status_code == 200:
            return "Successful Response" + "\n\n" + json.dumps(response.json(), indent=2)
        elif response.status_code == 422:
    ...
    def get_movie_by_id_comment_by_cid_movies__id__comments__cid__get(id: int, cid: int) -> dict:
        ...
        response = requests.get(
            MoviesApi.BaseUrl + f"/movies/{id}/comments/{cid}", headers=MoviesApi.HttpHeader
        )
        if response.status_code == 200:
            return "Successful Response" + "\n\n" + json.dumps(response.json(), indent=2)
        elif response.status_code == 422:
