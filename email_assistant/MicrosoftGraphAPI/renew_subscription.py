...
def renew_subscription(subscription_id):
    ...
    expiration_datetime = (datetime.now(UTC) + timedelta(days=3)).replace(
        microsecond=0
    ).isoformat() + "Z"
    headers = {
    ...
    subscription_url = f"{api_url}/subscriptions/{subscription_id}"
    ...
    if response.status_code == 200:
        ...
        raise Exception(
            f"Erro ao renovar a assinatura do webhook: {response.status_code} - {response.text}"
        )
